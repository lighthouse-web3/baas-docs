...
Your backup is now in `./db-dumps`. Construct the SDK client with the API key from step 1 and upload the directory as a snapshot. This reads `BACKUPDATA_API_KEY` and `BACKUPDATA_WORKSPACE_ID` from the environment (exported in step 1):
